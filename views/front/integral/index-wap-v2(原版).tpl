...
<head>
    ...
    <script src="{{static_front}}/static/front/integral/v2/wap/js/respond.min.js"></script>
    <link rel="shortcut icon" href="{{static_front}}/static/img/favicon.ico" type="image/x-icon">
    <link rel="stylesheet" href="{{static_front}}/static/front/integral/v2/wap/css/common.css">
    <link rel="stylesheet" href="{{static_front}}/static/front/query/skin-purple.css">
</head>
<body>
<header>
    <div>
        <ul id="accordion" class="accordion">
            <li>
                ...
                <ul class="submenu">
                    ...
                    {{if .rechargeSite}}
                    <li><a href="{{.rechargeSite}}" target="_blank">快速充值中心</a></li>{{end}}
                    {{if .officialPartner}}
                    <li><a href="{{.officialPartner}}" target="_blank">合作经营</a></li>{{end}}
                    {{if .officialRegist}}
                    <li><a href="{{.officialRegist}}" target="_blank">免费注册</a></li>{{end}}
                    {{if .officialPromot}}
                    <li><a href="{{.officialPromot}}" target="_blank">活动优惠</a></li>{{end}}
...
<div class="logined"><span>会员</span><strong id="username"></strong><span>当前积分</span><strong id="lotNums"></strong>
    <button onclick="exit()" class="btn exit-btn">退出</button>
</div>
<div class="main">
    <div class="container">
                    ...
                    <ul>
                        {{range $i, $v := .giftList}}
                            <li style="background: url({{$v.Photo}}) no-repeat center;">
                                <p><em>{{$v.Name}}</em></p>
                                <font>{{$v.Content}}</font>
                                <span style="color:#FFF600 ">{{$v.Seq}} 积分</span>
...
<script>
    ...
    $(function () {
        ...
        $(".tcsub").click(function () {
            ...
            $.post('{{urlfor "DoorController.Login"}}', {
                gid: $('#gid').val(),
                        ...
                        switch (obj.code) {
                            ...
                            case 1:
                                ...
                                layer.open({
                                    ...
                                    shadeClose: true,
                                    skin: 'layui-layer-nobg',
                                    content: '<div style="width:6rem; height:2rem; overflow:hidden; background-color: #239056; text-align: center; padding-top: 1rem;">您当前有<span>' + obj.data.lotteryNums + '</span>积分</div>',
                                    end: function () {
                                        $(".tcsub").unbind();
                                    }
                                });
