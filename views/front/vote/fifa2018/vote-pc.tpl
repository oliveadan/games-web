...
<div class="container">
	<div class="vote-table">
		<table>
            ...
            <tr>
				...
				{{if eq $i 0}}
				<td rowspan="{{$.voteItemList|len}}" style="font-size: 30px;color: red;">{{$.prize}}</td>
				{{end}}
            </tr>
			{{end}}
        </table>
	...
	<div class="form-div">
		...
		<div class="vote-share">立即分享获得更多投票机会！<button type="button" class="share-btn">点击分享</button></div>
     ...
     <div class="Field_num">
           <div class="main_bg">
             ...
             <div class="detail_box clearfix" style="width: 800px;">
	...
	<div class="tab1 tab_body" style="padding: 20px 0;color: #fff;font-size:16px;">
		{{str2html .gameRule}}
	</div>
	<div class="tab2 tab_body" style="padding: 20px 0;color: #fff;font-size:16px;">
...
<script src="/static/front/js/clipboard.min.js"></script>
<script>
$(function () {
	...
	$('.share-btn').click(function () {
        ...
        var iload = layer.load();
        $.ajax({
            url: '{{urlfor "ShareController.Post"}}',
            type: "post",
			data: {"gid": $("#gid").val(), "account": account},
            success: function (info) {
                layer.close(iload);
                if (info.code == 1) {
					var shareHtml = '<div id="shareContent" class="share-content">'
						+ '<p class="tips">' + info.data.tips + '</p>'
						+ '<p class="redirect">'+ info.data.redirect + '</p>'
						+ '</div>'
						+ '<div style="text-align:center;"><img width="100" src="data:image/png;base64,'+info.data.qrcode+'"></div>'
						+ '<div><span>复制链接或保存二维码分享！</span><button class="click-copy" data-clipboard-target="#shareContent">点击复制</button></div>';
					layer.open({
						title: '分享活动',
						btn: [],
						content: shareHtml
	...
	var clipboard = new ClipboardJS('.click-copy');
	clipboard.on('success', function(e) {
		//layer.close(shareDialog);
	    layer.tips("复制成功，快去分享吧！", ".click-copy", {tips:1});
		e.clearSelection();
	});
	clipboard.on('error', function(e) {
	    layer.tips("复制失败，请手动选择复制。", ".click-copy", {tips:1});
	});
});
</script>
</body>
