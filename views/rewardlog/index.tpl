	...
	<div class="layui-body">
	    ...
	    <div class="layui-tab layui-tab-brief">
	        <ul class="layui-tab-title">
                ...
                <li><a id="import" lay-data="{url: '{{urlfor "RewardLogIndexController.Import"}}'}"
                       href='javascript:void(0);'>批量导入</a></li>
	        </ul>
	        <div class="layui-tab-content">
				<form class="layui-form layui-form-pane form-container" style="max-width: 1500px;" action='{{urlfor "RewardLogIndexController.get"}}' method="get">
					...
					<div class="layui-inline">
						<div class="layui-input-inline">
							<select name="Deleted">
								<option value="0" {{if eq 0 .condArr.deleted}}selected="selected"{{end}}>未删除</option>
								<option value="1" {{if eq 1 .condArr.deleted}}selected="selected"{{end}}>已删除</option>
							</select>
						</div>
					</div>
					<div class="layui-inline">
	                ...
	                <div class="layui-inline" style="margin: 15px">
	                	...
	                	<a href='{{urlfor "RewardLogIndexController.Deliveredbatch"}}' class="layui-btn layui-btn-small ajax-batch">批量标记派送</a>
	                	<a href='{{urlfor "RewardLogIndexController.Delbatch"}}' class="layui-btn layui-btn-danger ajax-batch">删除所有</a>
	                </div>
	            </form>
	            <hr>
	                ...
	                <table class="layui-table">
						...
						{{range $index, $vo := .dataList}}
						    <tr>
		                        ...
		                        <td>{{if eq $vo.Delivered 1}}<span class="layui-badge layui-bg-green">已派送</span>{{else}}<span class="layui-badge layui-bg-red">未派送</span>{{end}}</td>
		                        <td>{{date $vo.DeliveredTime "Y-m-d H:i:s"}}</td>
		                        <td>
									{{if eq $vo.Delivered 0}}
									<a href='{{urlfor "RewardLogIndexController.Delivered" "id" $vo.Id "delivered" 1}}' class="layui-btn layui-btn-mini ajax-click">标记派送</a>
									{{else}}
									<a href='{{urlfor "RewardLogIndexController.Delivered" "id" $vo.Id "delivered" 0}}' class="layui-btn layui-btn-primary layui-btn-mini ajax-click">取消派送</a>
									{{end}}

		                            <a href='{{urlfor "RewardLogIndexController.Delone" "id" $vo.Id}}' class="layui-btn layui-btn-danger layui-btn-mini ajax-delete">删除</a>
		                        </td>
		                    </tr>
						{{else}}
