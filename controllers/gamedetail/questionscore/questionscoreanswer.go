...
type QuestionscoreAnswerIndexController struct {
	sysmanage.BaseController
}

func (this *QuestionscoreAnswerIndexController) Get() {
	//导出
	...
	o := orm.NewOrm()
	// 查询问题列表，包含选项
	var questList []Questionscore
	o.QueryTable(new(Questionscore)).Filter("GameId", gId).OrderBy("Seq", "Id").All(&questList, "Id", "Pid", "Seq", "Content", "ContentType")
	var tableHeadList = make([]Questionscore, 0) // 表格头部
	var opMap = make(map[int64]Questionscore)    // 选项ID和选项map
	...
	list, total := new(QuestionscoreAnswer).Paginate(page, limit)
	...
	if len(list) > 0 {
		...
		o.QueryTable(new(QuestionscoreAnswer)).Filter("GameId", gId).Filter("Pid__in", pids).OrderBy("-Pid").All(&list, "Pid", "Content", "ContentType", "CreateDate")
		for _, v := range list {
			if v.ContentType == 1 {
	...
	this.TplName = "gamedetail/questionscore/questionscoreanswer/index.tpl"
}

func (this *QuestionscoreAnswerIndexController) Export() {
	...
	if totalInt > limit {
		...
		for page = 2; page <= (page2 + 1); page++ {
			list1, _ := new(QuestionscoreAnswer).Paginate(page, limit)
			for _, v := range list1 {
				list = append(list, v)
			}
		}
	...
	if len(list) > 0 {
		...
		o.QueryTable(new(QuestionscoreAnswer)).Filter("GameId", gId).Filter("Pid__in", pids).OrderBy("-Pid").Limit(-1).All(&list, "Pid", "Content", "ContentType", "CreateDate")
		for _, v := range list {
			if v.ContentType == 1 {
				qids := strings.Split(v.Content, ",")
				for _, qidstr := range qids {
