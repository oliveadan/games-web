...
var Util = {
    ...
    WinningDetailsText: function() {
		...
		var gift = rllist[index%rlLen];
		index++;
		var dtl = "恭喜 "+gift.account+" 获得奖品 "+gift.gift+"!";
        $pisTexDom.html(dtl).fadeIn("slow",
        function() {
            Util.delay(1000,
            function() {
	...
	GetRlList: function() {
		$.post("/rllist",{"gid":$("#gameId").val()},
        function(data) {
            rllist = data;
		  	for(var item in rllist){
		    	rlLen++;
		  	}
        });
	},
    ...
    WishingSm: function() {
        ...
        function(data) {
            if (data.code > 0) {
                ...
                me.pis(data.msg);
                var userImg = "/static/img/logo.png";
    ...
    WishingPageList: function() {
        var http = $("#http").val()+"/static/front/wish/images/77dianzan.png";
        var me = this;
        ...
        function(data) {
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    htmlContent += '<tr class="addClassBg WishingMoreListAppend">' + '<td ' + classblue + '>' + data[i].Account + '</td>' + '<td ' + classblue + ' width="197.5px">' + data[i].Content + '</td>' + '<td ' + classblue + '><a href="javascript:Util.ThumbsUp(' + data[i].Id + ');" class="WishingPageListZan"><img src='+http+'>'+'<div class=dzs>'+ data[i].Thumbs +'</div>'+'</a></td>' + '</tr>';
                    // htmlContent += '<tr class="addClassBg WishingMoreListAppend">' + '<td ' + classblue + '>' + data[i].Account + '</td>' + '<td ' + classblue + '>' + data[i].Content + '</td>' + '<td ' + classblue + '>获得' + data[i].Thumbs + '个赞</td>' + '<td ' + classblue + '><a href="javascript:Util.ThumbsUp(' + data[i].Id + ');" class="WishingPageListZan"><img src='+http+'></a></td>' + '</tr>';
                }
